...
const TSUMEKO_PROBLEMS = [
    // === 一步题 ===
    {
        ...
        answer: [4, 4],
        explanation: '黑1落于(4,4)，同时打吃上方(4,5)和左边(4,2)两块白棋。白方无法同时救回两边，必死一块。双打吃是围棋最基本的吃子技巧之一。'
    },
    {
        name: '第2题：征子',
        desc: '黑先，用征子（扭羊头）吃白棋',
        ...
        answer: [3, 5],
        explanation: '黑1打吃(3,5)，白棋只能逃向(2,5)，之后黑棋步步紧逼将白棋逼到棋盘边缘全部吃掉。注意：征子前需确认征子路径上没有对方的接应棋子（征子有利）。'
    },
    {
        name: '第3题：做活',
        desc: '黑先，做两只眼活棋',
        ...
        answer: [2, 3],
        explanation: '黑1落于(2,3)，将左右两块黑棋连接起来，同时形成两只完整的眼位（左侧和右侧各一只）。两块黑棋连通后白方无法分断，黑棋净活。'
    },
    {
        name: '第4题：扑杀',
        desc: '黑先，用扑破坏白棋眼位',
        ...
        answer: [1, 2],
        explanation: '黑1扑入(1,2)是妙手！白棋提掉黑1后自紧一气，眼位被破坏。这是经典的"扑杀"手筋——故意送一子让对方提掉，反而缩小对方眼位，最终白棋因眼位不足而死。'
    },
    {
        name: '第5题：接不归',
        desc: '黑先，利用接不归吃白',
        ...
        answer: [2, 1],
        explanation: '黑1断打(2,1)！白棋右边三子(2,3)(3,2)(4,1)被打吃，而白棋试图连接时黑棋可顺势提掉。白棋气紧无法接回，形成"接不归"——想接却接不回去。'
    },
    {
        name: '第6题：倒扑',
        desc: '黑先，送一子然后倒扑吃白',
        ...
        answer: [1, 3],
        explanation: '黑1送吃(1,3)是倒扑手筋！白棋提掉这枚黑子后，黑棋立刻在原位回提，反而吃掉白棋(2,3)(2,2)等数子。倒扑的精髓在于"先舍后得"。'
    },
    {
        name: '第7题：枷吃',
        desc: '黑先，用枷吃（虚枷）吃白',
        ...
        answer: [3, 2],
        explanation: '黑1虚枷(3,2)！虽然没有直接打吃白棋，但将白棋(3,3)和(4,2)两块弱棋罩在网中。白棋无论从哪个方向逃都会撞到黑棋的包围圈，这是典型的"枷吃"（也叫虚枷、罩吃）。'
    },
    {
        name: '第8题：角部做活',
        desc: '黑先，在角上做活',
        ...
        answer: [2, 2],
        explanation: '黑1占据(2,2)是角部做活的要点！这一步同时扩张了黑棋的眼位空间，在角上形成了两只确定无疑的眼。角部死活中，"二·2"位置常常是做活的关键。'
    },
    // === 多步题 ===
    {
        name: '第9题：边上对杀',
        ...
        answer: [1, 1],
        sequence: [[1,1],[2,0],[2,2]],
        explanation: '黑1紧气(1,1)，白2只能提(2,0)，黑3再紧(2,2)彻底收气。黑棋比白棋快一气，最终白棋被吃。对杀中"气"的计算是关键——先数清楚双方各有几口气，再决定进攻还是防守。'
    },
    {
        name: '第10题：劫争',
        desc: '黑先，制造劫争吃白角',
        ...
        explanation: '黑1扑(0,1)制造劫争！白2提(0,0)，黑3提回(1,0)形成打劫。白方需要找劫材才能回提。角部的劫争非常常见，掌握开劫的技巧是围棋进阶的必修课。注意：打劫不能立刻回提，需要先在别处下一手（找劫材）。'
    },
    {
        name: '第11题：滚打包收',
        desc: '黑先，连续滚打收紧白气',
        ...
        answer: [4, 1],
        sequence: [[4,1],[3,1],[4,0]],
        explanation: '黑1打吃(4,1)，白2粘(3,1)，黑3继续打(4,0)……黑棋像滚雪球一样连续打吃白棋，将白棋的气越收越紧，最终将整块白棋"包收"吃掉。滚打包收是气势磅礴的连续进攻手段！'
    },
    {
        name: '第12题：金鸡独立',
        desc: '黑先，利用金鸡独立杀白',
        ...
        answer: [1, 1],
        sequence: [[1,1],[1,0],[2,0]],
        explanation: '黑1立下(1,1)形成"金鸡独立"！此时白棋两边都不入子——左侧气紧、右侧被黑棋控制，白方无法同时处理两边。黑3之后白棋气尽被吃。金鸡独立是边角对杀中的经典杀招。'
    }
];
...
class GoPractice {
    ...
    filterProblems() {
        if (this.category === 'all') {
            ...
            this.filteredProblems = TSUMEKO_PROBLEMS.filter(p => p.category === this.category);
        }
    }

    getPuzzle(index) {
        if (this.filteredProblems.length === 0) return null;
        if (index < 0) index = this.filteredProblems.length - 1;
        if (index >= this.filteredProblems.length) index = 0;
        return this.filteredProblems[index];
    }

    navPuzzle(dir) {
        const newIdx = this.currentPuzzle + dir;
        if (newIdx < 0 || newIdx >= this.filteredProblems.length) return;
        this.loadPuzzle(newIdx);
    }

    loadPuzzle(index) {
        if (index < 0 || index >= this.filteredProblems.length) return;
        this.currentPuzzle = index;
        this.solved = false;
        this.stepIndex = 0;

        const puzzle = this.filteredProblems[index];
        const total = this.filteredProblems.length;
        const globalIdx = TSUMEKO_PROBLEMS.indexOf(puzzle) + 1;
        ...
        document.getElementById('practiceTitle').textContent = `【${puzzle.category}】${puzzle.name} (${index + 1}/${total})`;
    ...
    handleClick(e) {
        ...
        const puzzle = this.filteredProblems[this.currentPuzzle];
        ...
        setTimeout(() => {
            if (row === ar && col === ac) {
                ...
                // 展示解析
                this.showExplanation(puzzle);

                // 多步题演示后续步骤
                if (puzzle.sequence && puzzle.sequence.length > 0) {
                    this.demoSequence(puzzle);
                }
            } else {
    ...
    showExplanation(puzzle) {
        const expEl = document.getElementById('practiceExplanation');
        if (puzzle.explanation) {
            const categoryLabel = puzzle.category === '一步题' ? '⚡ 一步题' : '📋 多步题';
            expEl.innerHTML = `<strong>${categoryLabel} · 解析：</strong>${puzzle.explanation}`;
    ...
    demoSequence(puzzle) {
        ...
        puzzle.sequence.forEach(([r, c], i) => {
            setTimeout(() => {
                const player = i % 2 === 0 ? opponent : color;
                this.board.board[r][c] = player;
